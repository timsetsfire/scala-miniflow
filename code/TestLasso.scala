...
val data = scala.io.Source.fromFile("c:/users/whittakert/desktop/nd4s/data/mnist_test.csv").getLines.toArray.map{ _.split(",").map{ _.toDouble} }

// val test = Nd4j.readNumpy("c:/users/whittakert/desktop/nd4s/data/mnist_test.csv", ",")

implicit val graph = MutMap[Node, ArrayBuffer[Node]]()

// val args = "sigmoid"
val x = new Input( (None, 784) )
val y = new Input()
val h1 = new ReLU(x, (784, 10) )
// val s1 = new SoftMax(x, (100, 10) )

val b = new Input()

def fisherYates[T](t: Array[T]): Array[T] = {
...
val y_ = Nd4j.zeros(batchSize, classes)

val dataBatch = Random.shuffle(data).take(batchSize)
val digits::pixels = dataBatch.transpose

for(i <- 0 until batchSize) {
  val ind = digits(i).toInt
  y_(i, ind) = 1d
}

val x_ = pixels.transpose.toNDArray
...
//
//
// val x = pixels.transpose.toNDArray
//
// Nd4j.shuffle(data, 1)
...
// val trainables = List(w,b)
//
//
// val ohe = Nd4j.zeros(batches, classes)
// for(i <- until batches) {
//   val ind = y(i, 0).toInt
//   val ohe(i, ind) = 1d
// }
